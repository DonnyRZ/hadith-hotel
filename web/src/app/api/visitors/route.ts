...
async function geolocate(ip: string) {
  const template =
    process.env.VISITOR_GEOLOOKUP_URL || "https://ipwho.is/{ip}";
  ...
  try {
    ...
    if (!result.ok || location.success === false) return null;
    if (typeof location.city !== "string") return null;
    if (!isMajorCity(location.city.trim())) return null;
...
function needsGeolocation(geoCheckedAt: Date | null | undefined) {
  if (!geoCheckedAt) return true;
  return geoCheckedAt.getTime() < Date.now() - GEO_REFRESH_MS;
}
...
async function visitorOverview(): Promise<Overview> {
  ...
  const topCities: CityMetric[] = topGroups
  ...
  return {
    count,
    activeVisitors,
    cities: cityGroups.filter((row) => row.city !== null).length,
    topCities,
  };
}
...
async function registerVisitor(ip: string, visitorHash: string) {
  ...
  const now = new Date();
  const geolocationNeeded = needsGeolocation(existing?.geoCheckedAt);
  const location = geolocationNeeded ? await geolocate(ip) : null;
  ...
  await prisma.websiteVisitor.create({
    data: {
      ...
      geoCheckedAt: geolocationNeeded ? now : null,
    },
  });
}

export async function GET() {
  if (!process.env.VISITOR_IP_HASH_SECRET) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (ip) {
      await registerVisitor(ip, await hashIp(ip));
    }
    return response(await visitorOverview());
  } catch (error) {
